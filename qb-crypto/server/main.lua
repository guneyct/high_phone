...
QBCore.Functions.CreateCallback('qb-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = QBCore.Functions.GetPlayer(source)
    if not tonumber(data.amount) then cb("ERROR") end
    local total_price = math.floor(tonumber(data.amount) * tonumber(Crypto.Worth["qbit"]))
    if Player.PlayerData.money.bank >= total_price then
        local CryptoData = {
            History = Crypto.History["qbit"],
            Worth = Crypto.Worth["qbit"],
            Portfolio = Player.PlayerData.money.crypto + tonumber(data.amount),
            WalletId = Player.PlayerData.metadata["walletid"],
        }
        ...
        Player.Functions.AddMoney('crypto', tonumber(data.amount))
        TriggerClientEvent("high_phone:updateBankData", Player.PlayerData.source)
        cb("SUCCESS")
    else
        cb("ERROR")
    end
...
QBCore.Functions.CreateCallback('qb-crypto:server:SellCrypto', function(source, cb, data)
    local Player = QBCore.Functions.GetPlayer(source)
    if not tonumber(data.amount) then cb("ERROR") end
    if Player.PlayerData.money.crypto >= tonumber(data.amount) then
        local CryptoData = {
            History = Crypto.History["qbit"],
            Worth = Crypto.Worth["qbit"],
            Portfolio = Player.PlayerData.money.crypto - tonumber(data.amount),
        ...
        Player.Functions.RemoveMoney('crypto', tonumber(data.amount))
        local amount = math.floor(tonumber(data.amount) * tonumber(Crypto.Worth["qbit"]))
