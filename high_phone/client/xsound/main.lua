...
CreateThread(function()
    if(Config.UseExternalxSound or Config.UseHigh3D or not Config.SyncedSounds) then return end

    local refresh = xConfig.RefreshTime
    ...
    while true do
        ...
        for k, v in pairs(soundInfo) do
            if v.position ~= nil and v.isDynamic then
                if #(v.position - playerPos) < v.distance + xConfig.distanceBeforeUpdatingPos then
